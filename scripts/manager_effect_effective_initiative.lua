	...
	if OptionsManager.isOption("HRIR", "on")  then
		if User.getRulesetName() == "5E" and RRRollManager and not OptionsManager.isOption("EFFECTIVE_INITIATIVE", "off") then
			...
			for _, nodeCT in pairs(ctEntries) do
				...
				if OptionsManager.isOption("EFFECTIVE_INITIATIVE", "all") or (OptionsManager.isOption("EFFECTIVE_INITIATIVE", "pcs") and (ActorManager.isPC(nodeCT) or bCohort)) then
...
function onInit()
	if Session.IsHost then
		...
		if  User.getRulesetName() == "5E" then
			...
			OptionsManager.registerOption2("EFFECTIVE_INITIATIVE", false, "option_effective_initiative",
			"option_ei_rr", "option_entry_cycler",
			{
				labels = "PCs Only|All",
				values = "pcs|all",
