...
function iconify()
{
    ...
    if [ -n "$filename" ] && [[ $filename == *.svg ]]; then
        ...
        "$convert" "$filename_png" -background none -gravity center -extent '112%' -write mpr:input -fill "$outline" -channel RGB -colorize 100 +channel -morphology dilate:4 disk -blur 0x3 mpr:input -composite "$filename_png"
